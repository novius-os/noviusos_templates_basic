<?php

// Generated on 03/12/2013 16:06:58

// 5 out of 5 messages are translated (100%).
// 18 out of 18 words are translated (100%).

return array(
    #: views/subviews/page_contexts.view.php:24
    'This page also exists in: {{contexts}}' => 'Эта страница также присутствует в: {{contexts}}',

    #: views/subviews/footer.view.php:16
    'Footer link' => 'Ссылка в подвале сайта',

    #: views/subviews/footer.view.php:17
    'Another footer link' => 'Еще одна ссылка в подвале сайта',

    #: views/subviews/footer.view.php:18
    'Powered by Novius OS' => 'На платформе Novius OS',

    #: config/template.config.php:15
    'My super website' => 'Мой крутейший сайт',

);
